...
var search = function(nums, target) {
  ...
  while (left <= right) {
      let mid = Math.floor((left + right) / 2); // Calcular el índice medio
...
var searchMatrix = function(matrix, target) {
  ...
  while (left <= right) {
      ...
      let row = Math.floor(mid / n); // Convertir índice lineal a fila
      let col = mid % n; // Convertir índice lineal a columna
